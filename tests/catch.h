...
namespace Catch {
    ...
    private:
        ...
        // Adds the current pattern as a tests name
        void addNamePattern();
    ...
    namespace Benchmark {
        struct Benchmark {
            ...
            ExecutionPlan<FloatDuration<Clock>> prepare(const IConfig &cfg, Environment<FloatDuration<Clock>> env) const {
                ...
                auto&& tests = Detail::run_for_at_least<Clock>(std::chrono::duration_cast<ClockDuration<Clock>>(run_time), 1, fun);
                int new_iters = static_cast<int>(std::ceil(min_time * tests.iterations / tests.elapsed));
                return { new_iters, tests.elapsed / tests.iterations * new_iters * cfg.benchmarkSamples(), fun, std::chrono::duration_cast<FloatDuration<Clock>>(cfg.benchmarkWarmupTime()), Detail::warmup_iterations };
            }
...
#else
            CATCH_ERROR( "Test failure requires aborting tests!" );
#endif
        }
    }
    ...
    clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto cli
            ...
            | Opt( config.benchmarkWarmupTime, "benchmarkWarmupTime" )
                ...
                ( "amount of time in milliseconds spent on warming up each tests (default: 100)" )
...
namespace Catch {
    ...
    void Session::libIdentify() {
        Catch::cout()
                << std::left << std::setw(16) << "description: " << "A Catch2 tests executable\n"
